...
### Bad: модель перечитывает то, что уже изучила в этой сессии
### Why: без персистентного транскрипта каждый ход начинался «с нуля» — те же get_player_sheet / get_scene_state / read_document вызывались повторно (до 6 дублей на батч = лишние LLM-вызовы). Старая двухфазная схема лечила это костылём study-summary.
### Good: весь раунд пишется в `AgentTranscript` и реконструируется в контекст каждого следующего хода — изученное уже в контексте, перечитывать не нужно. См. G40.

### Bad: единый кап лимита чтения для GM и Builder
### Why: кап 8000 на билдере заставляет модель дробить большой документ (25 КБ) на 4 части — 4 LLM-вызова на один документ вместо 1, билдер замедляется в 3-4 раза. GM не создаёт документы — ему кап полезен; билдеру нужен полный текст.
### Good: GM `read_document` — `Math.min(limit ?? 3000, 8000)`; Builder `read_document` — полное чтение при отсутствии offset/limit. См. G41.

### Bad: brain-секция 25 КБ одним файлом, индекс со сводками механик
### Why: get_brain/read_document тянет секцию целиком в контекст (6K токенов за раз), индекс дублирует секции — каждый вызов модели платит за огромный прелоад. GM «умнеет», но ответ идёт минуты.
### Good: секции мозга ≤6-7 КБ (разбивать на под-секции), индекс 3-5 КБ только навигация+политика, одна тема — в одном месте, дубли запрещены. Правила в гайде билдера. См. G42.

### Bad: передавать агенту output тула с Prisma-объектами Date
### Why: ai@7 валидирует результат тула как JSONValue; `updatedAt: Date` (Prisma) валится — шаг падает с «No output generated. Check the stream for errors» / errors.unknownError.
### Good: все тулсеты оборачиваются `wrapToolSet` (`tool-output.ts`) — JSON-roundtrip output'а (Date → ISO-строка). НЕ добавлять тул в раннер без обёртки. См. G44.

### Bad: retry пустого ответа рестартит с исходных сообщений без результатов текущего раунда
### Why: модель вызвала тулы, получила результаты, но не написала текст; retry с исходных сообщений отвечает «из памяти» (SRD) и теряет найденное.
### Good: один retry БЕЗ тулов, но с результатами текущего раунда через `stepsToModelMessages` (обёртка `{type:"json",value}`, `role:"tool"`). См. G30, G40.

### Bad: передавать агенту ID документов мозга без проверки активной игры
### Why: в БД лежит по копии индекса/секций на каждую мастер-игру. Правка ID из неактивной игры («DnD test 2») обновляет устаревший дубль, а активная игра («dnd test 3») остаётся со старыми битыми ссылками. Результат билдера не виден мастеру.
### Good: перед задачей — `getActiveGame().currentMasterId`, передавать ID именно из активной игры. См. G43.

### Bad: трейс аргументов тулов через `call.args`
### Why: AI SDK v7 кладёт аргументы tool call'а в `call.input`, поле `args` отсутствует → в TraceEvent пишется `args: {}` и анализировать вызовы невозможно.
### Good: `JSON.stringify(call.input ?? call.args ?? {})` в `onStepFinish`/`makeStepLogger`. См. G39.
...
### Good: единый агентный цикл — модель обязана ВЫЗВАТЬ `present_roll_check` (тул), а не описать его; промпт прямо это требует. Пустой ответ после тулов — retry БЕЗ тулов по результатам раунда (`stepsToModelMessages`), затем fallback. См. G30.
