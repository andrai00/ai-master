...
- Never wrap markdown sections in code fences with language "markdown"/"md" — use plain headings/tables. Formulas go ONLY in a single "formula" code block at the TOP of the sheet: base inputs (name: number) + derived formulas (name = expression); inline values in the body use $name and auto-substitute. Formula errors show as "err" — never guess. After create_document/update_document check the returned formulaValidation and fix any formula errors (errors include the line number).
