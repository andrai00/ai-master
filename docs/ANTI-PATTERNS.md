...
### Good: `Document.content` — полный текст одной записью. Для импорта — `bulk_import_to_glossary` с `createMany` (один SQL-запрос на папку). Архив распаковывается `adm-zip` на стороне сервера.
...
### Good: ответ доставляется только через `send_reply`, а перед ним обязательный `review_draft` (журнал действий хода + факты по БД). Обещал бросок/заметку/изменение документа — действие должно быть в журнале. См. G30.
