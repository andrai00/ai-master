...
> **`.kilo/` в `.gitignore`** — это нормально и задумано. Директория `.kilo/` содержит локальную конфигурацию Kilo-агента (правила, команды, скиллы, user preferences), которая не является частью приложения и не должна попадать в репозиторий. Каждый разработчик может иметь свою локальную `.kilo/`. Worktree-изоляция **не применяется** к `.kilo/` — это общая директория, не привязанная к git-веткам.
...
|----------|---------|-----------|
| `docs/GOLDEN-RULES.md` | 20 незыблемых правил (G1..G20) | Высший |
...
- **Prisma** — `migrate dev` для структурных изменений, `generate` после изменений
...
- **Ручные манипуляции с БД** — `sqlite3` или `npx prisma db execute --stdin`
- **pnpm** — единственный пакетный менеджер проекта. `pnpm add`, `pnpm install`, `pnpm exec`. `package-lock.json` удалён.
- **Миграция Prisma** — до перезапуска dev-сервера: `pnpm exec prisma migrate dev --name <name> && pnpm exec prisma generate`
