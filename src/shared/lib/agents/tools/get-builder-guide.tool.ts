...
- To READ a specific section instead of the whole document: read_document(id, toc_only=true) → summary + toc, then read_document(id, anchor="<heading>") → ONLY that section (mode:'section', not the whole doc). Read the whole document only when the section is not enough.
