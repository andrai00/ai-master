...
> Правила нумеруются (G1..G30) для ссылок: «нарушено G8».
...
- **GM** (игровой/личный чат): `search_rules` (глоссарий), `get_brain` (мозг), `get_gm_notes` / `get_scene_state` (игровая память), `get_player_sheet` (данные игрока), `get_players`, `resolve_glossary_link`.
- **Builder**: BRAIN mode → глоссарий + мозг (`search_rules`, `get_brain`, файловый импорт); MEMORY mode → игровая память и данные игроков (`get_gm_notes`, `get_scene_state`, `get_player_sheet`), как в игровом чате.
...
1. **Pass 1 — «Изучи и спланируй»** (read-only тулы, `getPlanTools`). Модель изучает ситуацию (мозг-индекс первым) и возвращает план: `STUDY | RECORD | ROLLS | REPLY`.
2. **Pass 2 — «Выполни план»** (полный набор тулов). Модель выполняет запланированные действия (записи в память, листы, `present_roll_check`) и пишет финальный ответ.

Доставка: всегда `result.text`. Если текст пуст — один ретрай с коррекцией «напиши полный ответ», затем fallback-сообщение.

**Гейт «пустой вызов»:** если нет новых сообщений игроков и нет завершённых необработанных роллов — Pass 1 пропускается, делается один короткий вызов с пометкой «не придумывай действия — спроси игрока, что он хочет делать».
...
**Реализация:** `getPlanTools(kind)` — read-only набор (нет write/roll-тулов); Pass 2 — полный `getTools()`. Ретрай «пустого ответа» — только в Pass 2. Рекурсивный batch не нужен: серверные экшены отправки блокируют новые сообщения во время обработки (`isProcessing`, ключ `chat.processingBlocked`).
