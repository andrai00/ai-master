...
### Bad: Pass 2 (exec) перечитывает данные, которые Pass 1 (plan) уже изучил
### Why: дублирование чтения: get_player_sheet / get_scene_state / get_gm_notes / read_document вызываются повторно — до 6 дублей на батч, каждый = отдельный LLM-вызов (30-100 сек). Ответ мастера разрастается с 40 сек до 3+ минут.
### Good: `buildStudySummary(toolResults, allowedTools)` (`study-summary.ts`) — результаты read-тулов Pass 1 вставляются в Pass 2 как `## Study summary`, EXEC-промпт запрещает перечитывать документы из сводки. См. G40.
