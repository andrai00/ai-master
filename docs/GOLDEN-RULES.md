...
- **GM** (игровой/личный чат): `search_rules` (глоссарий), `get_brain` (мозг), `get_gm_notes` / `get_scene_state` (игровая память), `get_player_sheet` (данные игрока), `get_players`, `resolve_glossary_link`, `send_reply`, `review_draft`.
- **Builder**: BRAIN mode → глоссарий + мозг (`search_rules`, `get_brain`, файловый импорт); MEMORY mode → игровая память и данные игроков (`get_gm_notes`, `get_scene_state`, `get_player_sheet`), как в игровом чате. Плюс `send_reply`, `review_draft`.
...
## G30 — Ответ агента доставляется через send_reply после review_draft

**Суть:** агенты (GM общий/личный, Builder) доставляют ответ только через тул `send_reply(text)` — сообщение попадает в чат при его вызове. Перед `send_reply` агент обязан вызвать `review_draft`: он возвращает **журнал действий текущего хода** (какие тулы реально вызваны) и факты по БД (назначенные роллы, незакрытые результаты, последние изменения документов). Агент сверяет черновик с журналом и доделывает недостающее.

**Почему:** модели иногда пишут «Жми кнопку!» / «Бросай X (1d20+6)» текстом, не вызвав `present_roll_check` — кнопка не появляется. Явная доставка + самопроверка по фактам делает обещания выполнимыми. Журнал действий (`actionLedger`) хранится в `reply-tools.ts` и заполняется в `onStepFinish`.

**Реализация:** `send_reply` / `review_draft` зарегистрированы во всех трёх чатах; `review_draft` возвращает `actionsThisTurn` + роллы/изменения документов. Если модель закончила без `send_reply` — `result.text` сохраняется как fallback (ответ не теряется).
